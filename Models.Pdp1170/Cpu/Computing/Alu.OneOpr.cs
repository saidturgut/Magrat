...
public partial class Alu
{
    ...
    private static AluOutput COM(AluInput input)
    {
        ...
        var output = new AluOutput { Result = MaskHigh((ushort)(~input.A)), ByteMask = ByteMask.PRES_HIGH };
    ...
    {
        var output = new AluOutput { Result = MaskHigh((ushort)(MaskHigh(input.A) + 1)), ByteMask = ByteMask.PRES_HIGH };
    ...
    {
        input.A = MaskHigh(input.A);
        var output = new AluOutput { Result = MaskHigh((ushort)(0 - input.A)), ByteMask = ByteMask.PRES_HIGH };
    ...
    {
        input.A = MaskHigh(input.A);
        var flags = new Flags(input.Psw);
        uint result = (uint)(input.A + CarryIn(flags));
        var output = new AluOutput { Result = MaskHigh((ushort)result), ByteMask = ByteMask.PRES_HIGH };
    ...
    private static AluOutput SBC(AluInput input)
    {
        input.A = MaskHigh(input.A);
        var flags = new Flags(input.Psw);
        var output = new AluOutput { Result = MaskHigh((ushort)(input.A - CarryIn(flags))), ByteMask = ByteMask.PRES_HIGH };
    ...
    private static AluOutput ROR(AluInput input)
    {
        input.A = MaskHigh(input.A);
        var flags = new Flags(input.Psw);
        var output = new AluOutput { Result = MaskHigh((ushort)((input.A >> 1) | (flags.Carry ? x8000 : 0))), ByteMask = ByteMask.PRES_HIGH };
    ...
    private static AluOutput ROL(AluInput input)
    {
        input.A = MaskHigh(input.A);
        var flags = new Flags(input.Psw);
        var output = new AluOutput { Result = MaskHigh((ushort)((input.A << 1) | CarryIn(flags))), ByteMask = ByteMask.PRES_HIGH };
    ...
    {
        input.A = MaskHigh(input.A);
        var output = new AluOutput { Result = MaskHigh((ushort)((input.A >> 1) | (input.A & x8000))), ByteMask = ByteMask.PRES_HIGH };
        if (CarryBit(input.A, 1)) output.Flags |= (ushort)Flag.CARRY;
        if (OverflowBit(output.Result, CarryBit(input.A, 1))) output.Flags |= (ushort)Flag.OVERFLOW;
        return output;
    }
    ...
    {
        input.A = MaskHigh(input.A);
        var output = new AluOutput { Result = MaskHigh((ushort)(input.A << 1)), ByteMask = ByteMask.PRES_HIGH };
        if (CarryBit(input.A, x8000)) output.Flags |= (ushort)Flag.CARRY;
        if (OverflowBit(output.Result, CarryBit(input.A, x8000))) output.Flags |= (ushort)Flag.OVERFLOW;
        return output;
    }
    ...
    {
        var output = new AluOutput { Result = (ushort)(((input.A & 0x00FF) << 8) | ((input.A & 0xFF00) >> 8)), Custom = true };
